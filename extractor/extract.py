...
def get_args():
    ...
    parser.add_argument('--fractional', action='store_true', default=False),
...
def main_lp(args):
    ...
    # Create solver
    solver = pywraplp.Solver('simple_mip_program', pywraplp.Solver.CPLEX_LINEAR_PROGRAMMING)
    if args.num_thread != 1:
        print("Set number of threads to {}".format(args.num_thread))
        solver.SetNumThreads(args.num_thread)
    ...
    x = {}
    for j in range(num_nodes):
        x[j] = solver.Var(0, 1, False, 'x[%i]' % j)
